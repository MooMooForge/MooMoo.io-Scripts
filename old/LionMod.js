...
, function(e, t, n) {
    (function(t) {
        ...
        e.exports.nameY = 34,
        e.exports.skinColors = ["#ff4500", "#ff00d5", "#ffff00", "#ffd700", "#00ffff", "#ffffff", "#ff0000", "#ff0080", "#00ff00", "#0000ff"],
    ...
    function ri(e, t) {
        ...
        if (!n || t) {
            ...
            "apple" == e.name) {
                ...
                "healing pad" == e.name ? (r.fillStyle = "#00ffff",
                oi(0, 0, 2 * e.scale, 2 * e.scale, r),
                r.fill(),
                r.stroke(),
                r.fillStyle = "#0000ff",
                ci(0, 0, .65 * e.scale, 20, 4, r, !0)) : "spawn pad" == e.name ? (r.fillStyle = "#ff0000",
                oi(0, 0, 2 * e.scale, 2 * e.scale, r),
                r.fill(),
                r.stroke(),
                r.fillStyle = "#00ffff",
        ...
        function() {
            ...
            a = 0; a < W.length + Y.length; ++a)
                if ((_ = W[a] || Y[a - W.length]).visible && (10 != _.skinIndex || _ == R || _.team && _.team == R.team)) {
                    ...
                    be.fill(),
                    be.fillStyle = _ == R || _.team && _.team == R.team ? "#ffff00" : "red",
...
setTimeout( () => {
$('#menuContainer').append('Engine')
setTimeout( () => {
$('#menuContainer').append('Master')
setTimeout( () => {
...
settings.style = `position: absolute; top: 10px; left: 10px; z-index: 100000; width: 50px; height: 50px; cursor: pointer;`;
...
(() => {
	try {
		...
		const modules = new Collection([{
			id: "core",
			name: "Home",
			description: "This Script Maded By ENGINE MASTER and \z\D.KNIGHT/z/",
			required: true,
			init: () => {
				...
				mesLink.attr("href", "http://sandbox.moomoo.io/");
...
document.getElementById("mainMenu").style.backgroundImage = "url('https://i.ibb.co/jTKGDqc/725b06130dd35d4b1a629857871af073.jpg')";
(function() {
    var můjVar;
    var můjVar2;
    var můjVar3;
...
document.getElementById('loadingText').innerHTML = '💠 Load LioNMod 💠';
setTimeout( () => {
...
setInterval(() => {
setTimeout( () => {
document.getElementById('nameInput').placeholder = "🐺Xx_EngineGaming_xX🐺";
setTimeout( () => {
...
document.getElementById('diedText').innerHTML = '💀  GG :c 💀';
setInterval(() => {
...
setTimeout( () => {
document.getElementById("woodDisplay").style.color = "#d820da";
setTimeout( () => {
...
setTimeout( () => {
document.getElementById("scoreDisplay").style.color = "#d820da";
setTimeout( () => {
...
setTimeout( () => {
document.getElementById("foodDisplay").style.color = "#d820da";
setTimeout( () => {
...
document.getElementById("ageText").style.color = "cyan";
document.getElementById("pingDisplay").style.color = "cyan";
document.getElementById("leaderboard").style.color = "cyane";
setInterval(() => {
setTimeout( () => {
...
setTimeout( () => {
document.getElementById("killCounter").style.color = "darkblue";
setTimeout( () => {
...
setTimeout( () => {
document.getElementById("gameName").style.color = "darkblue";
setTimeout( () => {
...
document.getElementById("gameName").innerHTML = "💥ζ͜͡𝕃𝕚𝕠𝕟 𝕄𝕠𝕕💥"
...
setInterval(() => {
    if(BotToggle == 1) {
        ...
        doNewSend(["6", [4]]);
        doNewSend(["6", [15]]);
        doNewSend(["10", ["LioNMoD"]]);
        doNewSend(["ch", ["LioNMoD-Bot Best!"]]);
        doNewSend(["33", [nearestEnemyAngle]]);
        setTimeout( () => {
            doNewSend(["ch", ['GO AWAY MOTHER FUCKER']]);
            doNewSend(["c", [1]]);
        }, 5000);}
}, 1);
...
function handleMessage(m){
    ...
    if(item == "h" && data[1] == myPlayer.id) {
        if(data[2] < 100 && data[2] > 0 && healToggle1 == 1) {
            setTimeout( () => {
                place(foodType, null);
                place(foodType, null);
                place(foodType, null);
...
function boostSpike() {
    ...
    doNewSend(["ch", ["BoostSpike Liked You Nab >:D"]]);
...
const turretPlacer = repeater(85, () => {place(turretType)}, 0);
const spawnPlacer = repeater(67, () => {place(spawnpadType)}, 0);
...
document.addEventListener('keydown', (e)=>{
    ...
    if(e.keyCode == 19 && document.activeElement.id.toLowerCase() !== 'chatbox') {
    ...
    if(e.keyCode ==  145 && document.activeElement.id.toLowerCase() !== 'chatbox') {
                doNewSend(["ch", ["Engine Master#5743"]]);
    }
    if(e.keyCode == 192 && document.activeElement.id.toLowerCase() !== 'chatbox') {
        if (heal == false) {
            heal = true;
                doNewSend(["ch", ["SuperAutoHeal : OFF"]]);
           } else {
                            doNewSend(["ch", ["SuperAutoheal : ON"]]);
            heal = false;
    }
    }
    if(e.keyCode == 48 && document.activeElement.id.toLowerCase() !== 'chatbox') {
        if (autoBar == false) {
            autoBar = true;
                doNewSend(["ch", ["AutoBarbarianPut : OFF"]]);
           } else {
                            doNewSend(["ch", ["AutoBarbarianPut : ON"]]);
            autoBar = false;
    }
    }
    ...
    if(e.keyCode == 115 && document.activeElement.id.toLowerCase() !== 'chatbox') {
        if (autoaim == false) {
            autoaim = true;
        document.title = "𝕃𝕚𝕠𝕟 𝕄𝕠𝕕 AutoAim : ON";
           } else {
        document.title = "𝕃𝕚𝕠𝕟 𝕄𝕠𝕕 AutoAim : OFF";
            autoaim = false;
    }
    }
    if(e.keyCode == 36 && document.activeElement.id.toLowerCase() !== 'chatbox') {
        if (autoSamu == false) {
            autoSamu = true;
                doNewSend(["ch", ["AutoSamurai : ON"]]);
           } else {
                            doNewSend(["ch", ["AutoSamurai : OFF"]]);
               autoSamu = false;
    }
    }
    if (e.keyCode == 115 && document.activeElement.id.toLowerCase() !== 'chatbox') {
    ...
    if(e.keyCode == 9 && document.activeElement.id.toLowerCase() !== 'chatbox') {
       doNewSend(["ch", ["Warning Me is Sh*t! LMAO"]]);
    }
    ...
    if(e.keyCode == 37 && document.activeElement.id.toLowerCase() !== 'chatbox') { // for Exe+FastBow
        doNewSend(["6", [8]]);
        doNewSend(["6", [17]]);
        doNewSend(["6", [31]]);
        doNewSend(["6", [23]]);
...
function animate(space, chance) {
    ...
    let characters;
    if(space) {
        characters = 'ThisisHackBetterthantigermod';
    } else {
        characters = 'LioNMoD';
    }
    if(space) {
